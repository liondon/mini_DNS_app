...
url = "http://{}:{}/fibonacci?hostname={}&fs_port={}&number={}&as_ip={}&as_port={}".format(US_ip, US_port, FS_hostname, FS_port, number, AS_ip, AS_port)
response = requests.get(url, timeout=2.50)
print(response.status_code, response.text)

## US: bad request: response 404
url = "http://{}:{}/fibonacci?hostname={}&fs_port={}&number={}&as_ip={}".format(US_ip, US_port, FS_hostname, FS_port, number, AS_ip, AS_port)
response = requests.get(url, timeout=2.50)
print(response.status_code, response.text)
